...
export const getProducts = asyncHandler(async (req: Request, res: Response): Promise<void> => {
  const products = await Product.find({ isActive: { $ne: false } })
    .select(
      '_id name image description ingredients howToUse category brand healthGoal price oldPrice discountPercentage rating numReviews countInStock reviews offer'
    )
    .lean();
